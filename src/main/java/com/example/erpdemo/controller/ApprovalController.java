...
import com.example.erpdemo.util.DateUtil;
...
public class ApprovalController {
    ...
    public void initialize() {
        ...
        // --- Genişlik kilitleme / sürükleme kapalı
        ...
        // Tablo içi davranışlar (çift tık → görüntüle)
        ...
        // Tablo dışına tıklama → seçimi temizle
    ...
    private void handleView() {
        ...
        try {
            FXMLLoader fxml = new FXMLLoader(getClass().getResource("/fxml/view-request.fxml"));
    ...
    private void handleApprove() {
        ...
        Async.runVoid(
                // background
                () -> requestService.approve(sel.getId(), HelloApplication.getLoggedInUserId()),
                // success
                () -> {
    ...
    private void handleReject() {
        ...
        setBusy(true);
        Async.runVoid(
                () -> requestService.reject(sel.getId(), HelloApplication.getLoggedInUserId()),
                () -> {
    ...
    /** Bekleyen talepleri yükle – müşteri adı servis tarafından geliyor. */
    private void refresh() {
        setBusy(true);
        Async.run(
                () -> requestService.getPendingSummaries(),
                list -> {
    ...
    private static java.sql.SQLException toSql(Throwable t) {
        if (t instanceof java.sql.SQLException se) return se;
        Throwable c = t.getCause();
        while (c != null && c != t) {
            if (c instanceof java.sql.SQLException se) return se;
            c = c.getCause();
        }
        return new java.sql.SQLException(t.getMessage(), t);
    }

    /** Tablo satırı modeli (UI için düz DTO) */
